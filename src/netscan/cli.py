...
from .network import NetworkDetectionError, get_network_cidr, scan_network

app = typer.Typer(help="Scan your local network and list discovered devices.")
console = Console()


def _run_scan(cidr: Optional[str], interface: Optional[str], timeout: float, retry: int) -> None:
    try:
    ...
    devices = scan_network(target_cidr, interface=interface, timeout=timeout, retry=retry)
...
def main(
    ...
    if ctx.invoked_subcommand is None:
        _run_scan(cidr, interface, timeout, retry)
